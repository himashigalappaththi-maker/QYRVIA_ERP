  ...
  return {
    async getRate(req, res, next) {
      try {
        ...
        okRead(res, req, await revenue.getRate(ctxOf(req), {
    ...
    async rateGrid(req, res, next) {
      try {
        ...
        okRead(res, req, await revenue.generateRateGrid(ctxOf(req), { roomTypeId: q.room_type_id, dateFrom: q.date_from, dateTo: q.date_to }));
      } catch (e) { if (/required|not_found/.test(e.message)) return fail(res, req, e.message); next(e); }
    },
    async forecast(req, res, next) {
      try {
        ...
        okRead(res, req, await revenue.getForecast(ctxOf(req), { dateFrom: q.date_from, dateTo: q.date_to }));
      } catch (e) { next(e); }
    },
    async kpis(req, res, next) {
      try { okRead(res, req, await revenue.getRevenueKPIs(ctxOf(req), { dateFrom: req.query.date_from, dateTo: req.query.date_to })); }
      catch (e) { next(e); }
    },
    async dashboard(req, res, next) {
      try { okRead(res, req, await revenue.getRevenueDashboard(ctxOf(req), { dateFrom: req.query.date_from, dateTo: req.query.date_to })); }
      catch (e) { next(e); }
    },
